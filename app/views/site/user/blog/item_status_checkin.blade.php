    ...
    <div class="col-md-12 col-none-padding person-content-article">
        ...
        <div class="row margin-none">
            <section class="article-img-text clearfix content-article-wrapper">
                <img class="avatar-pad2" src="{{$location->avatar}}" alt="">

                <div class="text-algin-img">
                    <header>
                        <a href="{{$location->url()}}"><h2> {{$location->name}}</h2></a>
                        <h3>{{ $userAuthor['username']}} {{ $userAuthor['text_status']}}</h3>
                    </header>
                    <article>
        ...
        <!-- hinh anh -->
        <?php
        $list_album=PostMeta::where('post_id','=',$review->id)->where('meta_key','=','review_image')->get();
